...
private:
    ...
    void updateTrackersWithHungarian(const std::vector<cv::Rect>& detections,
                                     ...
                                     std::vector<bool>* matchedDetectionsOut = nullptr);
    ...
    // Revive LOST trackers with unmatched detections (IoU + ReID, ReID-only
    // fallback gated by reidAppearanceThresh). Returns per-detection matched mask.
    std::vector<bool> matchLostTrackers(const std::vector<cv::Rect>& detections,
                                        const std::vector<float>& confidences,
                                        const std::vector<cv::Mat>& features = {});
